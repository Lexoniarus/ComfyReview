...
from services.comfy_client_core.discovery import get_checkpoints, get_samplers, get_schedulers
from services.comfy_client_core.http import http_json
from services.comfy_client_core.patching import patch_workflow_for_run
from services.comfy_client_core.types import ComfyResponse
from services.comfy_client_core.workflows import get_or_create_workflow_path, load_workflow
...
class ComfyClient:
    ...
    def _http_json(self, method: str, path: str, payload: Optional[dict] = None, timeout: int = 30) -> ComfyResponse:
        return http_json(base_url=self.base_url, method=method, path=path, payload=payload, timeout=timeout)

    def get_or_create_workflow_path(self, character_name: str) -> Path:
        return get_or_create_workflow_path(
    ...
    def enqueue_prompt(self, workflow: Dict[str, Any]) -> ComfyResponse:
        return self._http_json("POST", "/prompt", payload={"prompt": workflow}, timeout=60)
    ...
    def enqueue_from_playground(
        ...
        *,
        ...
        """Enqueue one job to ComfyUI based on a character workflow blueprint."""
        try:
            ...
            response = self.enqueue_prompt(patched)

            if response.ok and response.response_json and "prompt_id" in response.response_json:
    ...
    def get_samplers(self) -> list:
        return get_samplers(lambda m, p, pl, t: self._http_json(m, p, pl, t))

    def get_schedulers(self) -> list:
        return get_schedulers(lambda m, p, pl, t: self._http_json(m, p, pl, t))

    def get_checkpoints(self) -> list:
        return get_checkpoints(
            lambda m, p, pl, t: self._http_json(m, p, pl, t),
            checkpoints_dir=Path(COMFYUI_CHECKPOINTS_DIR),
        )
